...
public interface EatingHabitRepository extends JpaRepository<EatingHabit, Long> {
    ...
    @Query(value = "SELECT e.total_count FROM eating_habit e WHERE e.child_id = (SELECT c.child_id FROM children c WHERE c.name = :childName AND c.phone_number = : memberNumber) AND e.date = :date ORDER BY e.created_at DESC LIMIT 1 ", nativeQuery = true)
    List<Integer> findTotalCountTopByDateAndChildrenNameOrderByCreatedAtDesc(String date, String childName, String memberNumber);

    @Query(value = "SELECT e.total_time FROM eating_habit e WHERE e.child_id = (SELECT c.child_id FROM children c WHERE c.name = :childName AND c.phone_number = : memberNumber) AND e.date = :date ORDER BY e.created_at DESC LIMIT 1 ", nativeQuery = true)
    List<Integer> findTotalTimeTopByDateAndChildrenNameOrderByCreatedAtDesc(String date, String childName, String memberNumber);

    @Query(value = "SELECT e.bite_count_with_mouth FROM eating_habit e WHERE e.child_id = (SELECT c.child_id FROM children c WHERE c.name = :childName AND c.phone_number = : memberNumber) AND e.date = :date ORDER BY e.created_at DESC LIMIT 1", nativeQuery = true)
