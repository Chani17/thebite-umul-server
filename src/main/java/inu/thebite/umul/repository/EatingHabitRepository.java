...
public interface EatingHabitRepository extends JpaRepository<EatingHabit, Long> {
    ...
    @Query(value = "SELECT e.total_count FROM eating_habit e inner join children c WHERE c.name = :childName AND e.date = :date ORDER BY e.created_at DESC LIMIT 1 ", nativeQuery = true)
    List<Integer> findTotalCountTopByDateAndChildrenNameOrderByCreatedAtDesc(String date, String childName);

    @Query(value = "SELECT e.total_time FROM eating_habit e inner join children c WHERE e.date = :date AND c.name = :childName ORDER BY e.created_at DESC LIMIT 1 ", nativeQuery = true)
    List<Integer> findTotalTimeTopByDateAndChildrenNameOrderByCreatedAtDesc(String date, String childName);

    @Query(value = "SELECT e.bite_count_with_mouth FROM eating_habit e inner join children c WHERE c.name = :childName AND e.date = :date ORDER BY e.created_at DESC LIMIT 1", nativeQuery = true)
